	...
	<div id="page">
		...
		${param.message}<br>
				...
				<table border="1">
					<tr>
						<td>순번</td>
						<%-- not cardNo, not cardPayNo, just No --%>
						<td>작성자</td>
						<td>작성일</td>
						<td>질문</td>
					</tr>
					<c:forEach var="q" items="${qnaList.pageList}" varStatus="status">
						...
						<c:if
							test="${(q.secret == '0') ||(q.qUser.id == user.id) ||(q.aUser.id == user.id)}">
							<tr>
								<td>${status.count}</td>
								<td>${q.qUser.id}</td>
								<td>${q.qDate}</td>
								<td><c:if test="${q.answer != null}">
										<a
											href="<c:url value='/normal/viewQnAList2.do'>
										<c:param name='ques' value='click' />
										<c:param name='saleNo' value='${q.normal.saleNo}' />
										<c:param name='quesNo' value='${q.qNo}' />
									</c:url>">
											${q.question}</a>
									</c:if> <c:if test="${q.answer == null}">
							${q.question}
						</c:if></td>
								<%-- 질문 클릭하면 해당 질문 답변 나옴 --%>
								...
								<c:if test="${(q.answer == NULL) && (user.id == q.aUser.id) }">
									<td><a
										href="<c:url value='/normal/answerQnA.do'>
										<c:param name='saleNo' value='${q.normal.saleNo}' />
